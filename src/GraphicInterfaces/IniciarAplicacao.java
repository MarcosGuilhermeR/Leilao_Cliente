...
import Controller.ProdutoLeilao;
...
public class IniciarAplicacao  {
    ...
    {
        JFrame contentPane = IniciarAplicacao.wrapframe;
    ...
    static void createAndShowGui() {
        ...
        IniciarAplicacao.wrapframe.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        IniciarAplicacao.wrapframe.getContentPane().add(mainPanel);
        IniciarAplicacao.wrapframe.pack();
        IniciarAplicacao.wrapframe.setLocationByPlatform(true);
        IniciarAplicacao.wrapframe.setSize(900, 650);
        IniciarAplicacao.wrapframe.setLocationRelativeTo(null);
        IniciarAplicacao.wrapframe.setVisible(true);
        IniciarAplicacao.wrapframe.setBackground(Color.LIGHT_GRAY);
   }
...
class MainWin extends JPanel {
   ...
   public MainWin(String msgS) {
        ...
        btncadPanel.add(new JButton(new AbstractAction("Cadastrar Produtos") {
            @Override
            public void actionPerformed(ActionEvent arg0) {
                CadastroDeProduto icad = new CadastroDeProduto();
                icad.show();
            }
        }));
                ...
                if(listaProdutosAtivos != null)
                {
                    ...
                    if(size > 0)
                    {
                        ...
                        {  
                            if(chave != null)  
                            {
                                ProdutoLeilao objeto = (ProdutoLeilao) deserializeHashMap.get(chave);
                                
                                rows[lines][0] = objeto.getCodigoProduto();
                                rows[lines][1] = objeto.getNomeProduto();
                                rows[lines][2] = objeto.getMaiorValor(); 
                                rows[lines][3] = objeto.getNomeVencedor();
                                rows[lines][4] = objeto.getNomeVendedor();
                                rows[lines][5] = "VISUALIZAR"; 
                                lines++;
